...
challengeController.toggleCompleted = async (req, res) => {
  try {
    const challenge = await Challenge.findOne({challenge: req.params.challengeName})
    if(challenge.completedBy.includes(req.body.username) == false) {
      let newList = challenge.completedBy
      newList.push(req.body.username)
      await Challenge.updateOne({challenge: req.params.challengeName}, {completedBy: newList})
      res.status(200)
